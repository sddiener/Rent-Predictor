...
def immonet_parser(page_source):
    ...
    for i, selObject in enumerate(selObjects): # extract info from apartments
        ...
        try:
            ...
            print('- Error extracting tags (selObj {}/{}): {}'.format(i, len(selObjects)-1, e))
...
for state in range(9,17):  # loop 16 federal states
    ...
    while True:
        ...
        df = immonet_parser(driver.page_source) # extract 6 cols: id, title, bullets, tags, price, area, rooms
        ...
        df.to_csv('data/state_{}/immo_data_state{}_page{}.csv'.format(state,state,page))
        #time.sleep(0.25)
